...
def convertDate(string):
    try:
        ...
        condate = condate.replace(minute=00)
    except ValueError:
...
def getBikeLoss(reader, day):
    ...
    while hasnext:
        try:
            row = reader.next()
            if convertDate(row['Start Date']) == day:
...
def getBikeGain(reader, day):
    ...
    while hasnext:
        try:
            row = reader.next()
            if convertDate(row['End Date']) == day:
...
#Generate xml file for all station by day
...
def writeXml(earliest, latest, mainreader, namelist):
    ...
    next = earliest
    while next <= latest:
        print "writing data for ", str(next.date())

        filename = next.date()
        root = et.Element("dailytripdata")
        root.set('version', '1.0')
        comment = et.Comment("Trip data for " + str(filename))
        root.append(comment)
        ...
        print "wrote to: " + str(filename) + ".xml"

        with open("Data/"+str(filename)+".xml", "w") as f:
            f.write(tree)
...
with open('bigsample.csv',
    ...
    while next.date < latest.date:
        print "Next:", str(next.date)
        next = next + timedelta(days=1)
    print "Latest:", str(latest.date)
